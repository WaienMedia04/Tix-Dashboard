    ...
    <div className="rounded-xl border border-border/70 bg-background p-3.5">
      <div className="flex items-center justify-between gap-2">
        ...
        {puedeCrear && (
          <button
            onClick={onCrear}
            className="flex items-center gap-1 rounded-full border border-border px-2 py-0.5 text-[11px] font-medium hover:bg-accent"
          >
      ...
      {!encuesta ? (
        <p className="mt-2 text-xs text-muted-foreground">Todavía no hay ninguna encuesta activa.</p>
      ) : (
        <div className="mt-2 space-y-1.5">
          <p className="text-sm font-medium text-foreground">{encuesta.pregunta}</p>
          {encuesta.opciones.map((opcion, i) => {
            ...
            return (
              <button
                ...
                disabled={votando}
                className={`relative w-full overflow-hidden rounded-md border px-2.5 py-1.5 text-left text-xs transition-colors disabled:opacity-70 ${
                  esMiVoto ? "border-primary" : "border-border"
                }`}
              >
                <div className="absolute inset-y-0 left-0 bg-primary/15" style={{ width: `${pct}%` }} />
                <div className="relative flex items-center justify-between gap-2">
                  <span className={esMiVoto ? "font-semibold text-primary" : "text-foreground"}>{opcion}</span>
