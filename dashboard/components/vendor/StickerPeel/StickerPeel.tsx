// @ts-nocheck
/* eslint-disable */
// Vendorizado de React Bits (https://reactbits.dev) — componente StickerPeel.
"use client";
    ...
    draggableInstanceRef.current = Draggable.create(target, {
      ...
      onDragEnd() {
        ...
        if (onPositionChange) {
          ...
          const posX = ((targetRect.left - boundsRect.left) / boundsRect.width) * 100;
          const posY = ((targetRect.top - boundsRect.top) / boundsRect.height) * 100;
          onPositionChange({ posX: Math.max(0, Math.min(100, posX)), posY: Math.max(0, Math.min(100, posY)) });
        }
      },
    })[0];
