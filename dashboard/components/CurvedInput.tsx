...
export interface CurvedInputProps {
  ...
  borderColor?: string;
  buttonColor?: string;
  /** Color stops (left→right) for a gradient fill on the submit button; overrides buttonColor when set. */
...
export default function CurvedInput({
  ...
  const buttonGradientId = `ci-btn-gradient-${uid}`;
  ...
  if (geom && layout) {
    ...
    const buttonFill = buttonGradient && buttonGradient.length > 1 ? `url(#${buttonGradientId})` : accentColor;
    ...
    content = (
      ...
      >
        <defs>
          ...
          {buttonGradient && buttonGradient.length > 1 && (
            <linearGradient id={buttonGradientId} x1="0%" y1="0%" x2="100%" y2="0%">
              {buttonGradient.map((stopColor, i) => (
                <stop key={i} offset={`${(i / (buttonGradient.length - 1)) * 100}%`} stopColor={stopColor} />
              ))}
            </linearGradient>
          )}
        </defs>
        ...
        {showButton && (
          ...
          >
            <path className="curved-input__button-bg" d={buttonPath} fill={buttonFill} />
